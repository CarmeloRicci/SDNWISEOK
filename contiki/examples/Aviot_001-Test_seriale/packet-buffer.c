...
/*----------------------------------------------------------------------------*/
  ...
  print_packet(packet_t* p)
  {
    ...
    printf("\n\tnet[%d] len[%d] ", p->header.net, p->header.len);
    printf("dest [");
    print_address(&(p->header.dst));
    printf("] source [");
    print_address(&(p->header.src));
    printf("] type[%d] ttl[%d] ", p->header.typ, p->header.ttl);
    printf(" nexthop [");
    print_address(&(p->header.nxh));
    printf("] payload [ ");
    for (i=0; i < (p->header.len - PLD_INDEX); ++i){
    ...
    void print_packet_MELO(packet_t* p) {
      uint16_t i = 0;
      
      printf("\n\nnet[%d] \nlen[%d] ", p->header.net, p->header.len);
      printf("\ndest [");
      print_address(&(p->header.dst));
      printf("] \nsource [");
      print_address(&(p->header.src));
      printf("] \ntype[%d] \nttl[%d] ", p->header.typ, p->header.ttl);
      printf("\nnexthop [");
      print_address(&(p->header.nxh));
      printf("] \npayload [ ");
      for (i=0; i < (p->header.len - PLD_INDEX); ++i){
