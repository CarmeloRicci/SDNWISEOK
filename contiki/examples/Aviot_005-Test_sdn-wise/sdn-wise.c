...
int uart_rx_callback(unsigned char c)
{
...
#if DEBUG_MELO
  //PRINTF("sdn-wise.c --- uart_rx_callback - Ricevo dati dalla seriale\n");
  ...
  if (uart_buffer_index == uart_buffer_expected)
  {
    ...
    //PRINTF("\n\nsdn-wise.c --- uart_rx_callback - Ho rivevuto e creato un pacchetto\n\n");
    if (p != NULL) {
...
#if !SINK
#if DEBUG_MELO
// ************* codice per nodo test

  //   uart_buffer[uart_buffer_index] = c;
  //   uart_buffer_index++;
  // if (uart_buffer_index == 80) {
  //   count++;
  //   printf("HO RICEVUTO LA FRAME N: %d\n",count);
...
// ************* codice per nodo sdnwise funzionante
    uart_buffer[uart_buffer_index] = c;
    uart_buffer_index++;
    //printf("[[[ %d ]]]\n", uart_buffer_index);
    if (uart_buffer_index == 80) {
    ...
    //         packet_t *p = create_data_MELO3(1);
...
PROCESS_THREAD(main_proc, ev, data)
{
  ...
  while (1)
  {
    ...
    case RF_B_RECEIVE_EVENT:
      ...
      {
        conf.is_active = 1;
        process_post(&beacon_timer_proc, ACTIVATE_EVENT, (process_data_t)NULL); //ATTIVO I THERAD
    ...
    case RF_SEND_DATA_EVENT:
      ...
      //  }
        ...
        // //   match_packet(p);
        // //   rf_unicast_send( p );
        //   PRINTF("Creo il pacchetto di Open-Path\n");
        //   packet_t *p = create_data_MELO4(1,2);
        //   print_packet_MELO(p);
        //   if (p != NULL) {
        //       p->info.rssi = 0;
        //       PRINTF("Simulo di aver ricevuto il pacchetto di Open-Path\n");
        //       process_post(&main_proc, UART_RECEIVE_EVENT, (process_data_t)p);
        //       printf("ok! Vai\n");
        //     }
        //   }
        //   if(NNode2 > 15){
        //   packet_t *p = create_data_MELO1(1,50, 48, 48, (48+NNode2-14));
        //   match_packet(p);
        //   //rf_unicast_send(p);
        //   }
        //    NNode2++;
  ...
  while (1)
  {
    ...
    if (p != NULL)
    {
      ...
      if (!is_my_address(&(p->header.dst)))
      {
        ...
        //PRINTF("sdn-wise.c --- ho mandato un pacchetto in unicast\n");
      }
#if SINK
      else
      {
      ...
      printf("Pacchetto vuoto o memoria piena!\n");
    }
  }
  PROCESS_END();
}
