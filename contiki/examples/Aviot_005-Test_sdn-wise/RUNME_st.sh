...
make TARGET=$TARGET BOARD=$BOARD DEFINES=COOJA=0,SINK=1,_MY_ADDRESS=0,SDN_WISE_DEBUG=1,DEBUG_MELO=1
...
make TARGET=$TARGET BOARD=$BOARD DEFINES=COOJA=0,SINK=0,_MY_ADDRESS=4,SDN_WISE_DEBUG=1,DEBUG_MELO=1
arm-none-eabi-objcopy -O binary sdn-wise.$TARGET sdn-wise_2.$TARGET
mv sdn-wise_2.$TARGET cooja_firmwares/node_2.$TARGET 
make TARGET=$TARGET BOARD=$BOARD clean


make TARGET=$TARGET BOARD=$BOARD SENSORBOARD=$SENSORBOARD DEFINES=COOJA=0,SINK=0,_MY_ADDRESS=1,SDN_WISE_DEBUG=1
...
make TARGET=$TARGET BOARD=$BOARD SENSORBOARD=$SENSORBOARD DEFINES=COOJA=0,SINK=0,_MY_ADDRESS=2,SDN_WISE_DEBUG=1
