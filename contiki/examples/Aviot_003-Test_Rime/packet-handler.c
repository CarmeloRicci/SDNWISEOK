  ...
  void handle_packet(packet_t* p) {
    if (p->info.rssi >= conf.rssi_min && p->header.net == conf.my_net){
      ...
      if (p->header.typ == BEACON){
        ...
        #if DEBUG_MELO
              printf("--- handle_packet - Switch case tipo pacchetto %d\n",p->header.typ);
              printf("--- handle_packet - tipo pacchetto nuovo %d\n",DATAMELO);
        #endif
          switch (p->header.typ){
            case DATA:
            PRINTF("[PHD]: Data\n");
...
/*----------------------------------------------------------------------------*/
  ...
  handle_data(packet_t* p)
  {
    ...
    {     
      ...
      #if DEBUG_MELO
        printf("--- handle_data - Consumo il pacchetto: \n");
        print_address_MELO(&(p->header.dst),"Destinatario");
        print_address_MELO(&(p->header.src),"Mittente");
        print_packet_MELO(p);
      #endif
              ...
              print_packet_uart(p);
                #if DEBUG_MELO
                        printf("--- handle_data - Sono il Sink, stampo il pacchetto nella seriale\n");
                #endif
      #endif
  ...
  void  handle_open_path(packet_t* p)  {
    ...
    if (my_position > 0) { 
      ...
      uint8_t first = start;
        ...
        #if DEBUG_MELO
              PRINTF("--- packet-handler.c --- handle_open_path - avvio create_entry() - #001\n");
        #endif
    ...
    { 
      ...
      uint8_t last = end - ADDRESS_LENGTH;
        ...
        #if DEBUG_MELO
              PRINTF("--- packet-handler.c --- handle_open_path - avvio create_entry() - #002\n");
        #endif
...
/** @} */
  ...
  void handle_datamelo(packet_t* p) {
        ...
        rf_unicast_send(p);
    printf("\n\n--- handle_packet - handle_datamelo - Pacchetto inviato e deallocato @@@@\n\n");
        packet_deallocate(p);
  }
/*
  if ( !is_my_address(&(p->header.dst)) ) {  

      //PRINTF("[PHD] DATAMELO: Pacchetto audio ricevuto - IF\n");

//PRINTF("Pacchetto prima \n\n");
        //print_packet_MELO(p);

        p->header.nxh = conf.nxh_vs_sink;
        p->header.typ = DATA;
        //printf("--- handle_datamelo - Consumo il pacchetto: \n");
        //print_address_MELO(&(p->header.dst),"Destinatario");
        //print_address_MELO(&(p->header.src),"Mittente");
        //print_packet_MELO(p);

//PRINTF("[Pacchetto dopo \n\n");
        rf_unicast_send(p);
    printf("\n\n--- handle_packet - handle_datamelo - Pacchetto inviato e deallocato @@@@\n\n");
        packet_deallocate(p);
    } else {
      //PRINTF("[PHD] DATAMELO: Pacchetto audio ricevuto - ELSE\n");
      match_packet(p);
    }

    //printf("--- handle_packet - handle_datamelo - Pacchetto inviato e deallocato\n");
