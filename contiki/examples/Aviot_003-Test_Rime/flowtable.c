...
#endif
/*----------------------------------------------------------------------------*/
  LIST(flowtable); // LinkedList
  MEMB(entries_memb, entry_t, 10); // topo le strutture ma sono blocchi di memoria statici
...
/*----------------------------------------------------------------------------*/
  entry_t* create_entry(void) {
        #if DEBUG_MELO
              PRINTF("--- flowtable.c --- create_entry - avvio entry_allocate()\n");
        #endif
...
/*----------------------------------------------------------------------------*/  
  action_t* create_action(action_type_t type, uint8_t* array, uint8_t len) {
...
/*----------------------------------------------------------------------------*/    
  void add_window(entry_t* entry, window_t* w) {
    list_add(entry->windows,w);
  }
/*----------------------------------------------------------------------------*/    
  void add_action(entry_t* entry, action_t* a) {
    list_add(entry->actions,a); 
  }
/*----------------------------------------------------------------------------*/
  static void print_window(window_t* w) {
...
/*----------------------------------------------------------------------------*/
  void print_entry(entry_t* e) {
    ...
    action_t *a;
         #if DEBUG_MELO
              PRINTF("--- flowtable.c --- print_entry - inizio\n");
        #endif
    PRINTF("IF (");
    ...
    PRINTF("}[%d %d]", e->stats.ttl, e->stats.count);   
        #if DEBUG_MELO
              PRINTF("--- flowtable.c --- print_entry - fine\n");
        #endif
  }
/*----------------------------------------------------------------------------*/
  void print_flowtable(void) {
    entry_t *e;
        #if DEBUG_MELO
              PRINTF("--- flowtable.c --- print_flowtable - inizio\n");
        #endif
    for(e = list_head(flowtable); e != NULL; e = e->next) {
      ...
      PRINTF("\n");  
        ...
        #if DEBUG_MELO
              PRINTF("--- flowtable.c --- print_flowtable - fine\n");
        #endif
  }
/*----------------------------------------------------------------------------*/
  static window_t * window_allocate(void) {
...
/*----------------------------------------------------------------------------*/
  static void window_free(window_t *w) {
...
/*----------------------------------------------------------------------------*/
  static void byte_free(byte_t *b) {
...
/*----------------------------------------------------------------------------*/
  static void purge_bytes(list_t bytes) {
    byte_t *e;
    byte_t *next;
...
/*----------------------------------------------------------------------------*/
  static void action_free(action_t *a) {
    purge_bytes(a->bytes);
...
/*----------------------------------------------------------------------------*/
  static void purge_windows(list_t windows) {
    window_t *e;
    window_t *next;
...
/*----------------------------------------------------------------------------*/
  static void purge_actions(list_t actions) {
    action_t *e;
    action_t *next;
...
/*----------------------------------------------------------------------------*/
  static void entry_init(entry_t *e) {
    memset(e, 0, sizeof(*e));
    e->stats.ttl = 0;
    e->stats.count = 0; 
...
/*----------------------------------------------------------------------------*/
  static entry_t * entry_allocate(void) {
    entry_t *e;
        #if DEBUG_MELO
              PRINTF("--- flowtable.c --- entry_allocate\n");
        #endif
    ...
    if(e == NULL) {
      ...
      if(e == NULL) {
        #if DEBUG_MELO
              PRINTF("--- flowtable.c --- entry_allocate - failed\n");
        #endif
        ...
        #if DEBUG_MELO
              PRINTF("--- flowtable.c --- entry_allocate - success - avvio entry_init\n");
        #endif
    entry_init(e);
    return e;
  }
...
/*----------------------------------------------------------------------------*/
  window_t* get_window_from_array(uint8_t* array) {
...
/*----------------------------------------------------------------------------*/
  uint8_t get_array_from_window(uint8_t* array, window_t* w) {
...
/*----------------------------------------------------------------------------*/
  action_t* get_action_from_array(uint8_t* array, uint8_t action_size) {
    return create_action(array[0], &(array[1]), action_size-1);
  }
/*----------------------------------------------------------------------------*/
  uint8_t  get_array_from_action(uint8_t* array, action_t* a) {
    int i = 2;
    array[1] = a->type;
    byte_t *b;
    for(b = list_head(a->bytes); b != NULL; b = b->next) {
...
/*----------------------------------------------------------------------------*/
  entry_t*  get_entry_from_array(uint8_t* array, uint16_t length) {
...
/*----------------------------------------------------------------------------*/
  uint8_t get_array_from_entry(uint8_t* array, entry_t* e) {  
    uint8_t index = 1;
    array[0] = 0;
    window_t *w;
    action_t *a;
...
/*----------------------------------------------------------------------------*/
  uint8_t get_array_from_entry_id(uint8_t* array, int entry_id) {  
    int i = 0;
    uint8_t size = 0;
    entry_t *e;
    for(e = list_head(flowtable); e != NULL; e = e->next) {
...
/*----------------------------------------------------------------------------*/
  uint8_t window_cmp(window_t* a, window_t* b) {  
    print_window(a);
    PRINTF("\n");
    print_window(b);
    PRINTF("\n");
...
/*----------------------------------------------------------------------------*/
  uint8_t entry_cmp(entry_t* a, entry_t* b) {  
...
/*----------------------------------------------------------------------------*/
  void add_entry(entry_t* e) {
    ...
    entry_t* found = NULL;
         #if DEBUG_MELO
              PRINTF("--- flowtable.c --- add_entry - vedo se e' prevente gia'\n");
        #endif
    ...
    if (found != NULL){
        #if DEBUG_MELO
              PRINTF("--- flowtable.c --- add_entry - Si, e' gia' presente - attivo entry_free()\n");
        #endif
      entry_free(found);
    }
        #if DEBUG_MELO
              PRINTF("--- flowtable.c --- add_entry - Aggiungo l'entry alla flowtable - eseguo list_add(flowtable,e)\n");
        #endif
    list_add(flowtable,e);
  }
/*----------------------------------------------------------------------------*/
  void match_packet(packet_t* p) {
    entry_t *e;
    int found = 0;
    PRINTF("[FLT]: Matching Packet...\n");

      #if DEBUG_MELO
        PRINTF("--- flowtable.c --- match_packet - vedo se e' presente una regola nella flowtable\n");
      #endif
    ...
    for(e = list_head(flowtable); e != NULL; e = e->next) {
      ...
      if (found){
      #if DEBUG_MELO
        PRINTF("--- flowtable.c --- match_packet - corrispondenza trovata!\n");
      #endif
        break;
      }
    }
...
/*----------------------------------------------------------------------------*/
  int match_entry(packet_t* p, entry_t* e) {
    window_t *w;
    action_t *a;
    ...
    int actual = 0;
      ...
      #if DEBUG_MELO
        PRINTF("--- flowtable.c --- match_entry - sono nella funzione match_entry !\n");
      #endif
    ...
    for(w = list_head(e->windows); w != NULL; w = w->next) {
      ...
      #if DEBUG_MELO
        PRINTF("--- flowtable.c --- match_entry - ho cercato nella flowtable \n");
      #endif

    if (actual == target){
      PRINTF("[FLT]: Match Found!\n");
      #if DEBUG_MELO
        PRINTF("--- flowtable.c --- match_entry - corrispondenza trovata \n");
      #endif
      for(a = list_head(e->actions); a != NULL; a = a->next) {
      ...
      #if DEBUG_MELO
        PRINTF("--- flowtable.c --- match_entry - non ho trovato nessuna corrispondenza\n");
      #endif
      return 0;
    }  
  }
/*----------------------------------------------------------------------------*/
  static int compare(operator_t op, uint16_t lhs, uint16_t rhs) {
    switch (op) {
...
/*----------------------------------------------------------------------------*/
  static uint16_t get_operand(packet_t* p, uint8_t* r, operator_size_t s, operator_location_t l, uint16_t v) {
    uint8_t* ptr;
    int limit;
    uint8_t* a = (uint8_t*)p;
...
/*----------------------------------------------------------------------------*/
  int match_window(packet_t* p, uint8_t* s, window_t* w) {
...
/*----------------------------------------------------------------------------*/
  static uint16_t do_operation(set_operator_t op, uint16_t lhs, uint16_t rhs) {
    switch (op) {
...
/*----------------------------------------------------------------------------*/
  static void set_action(packet_t* p, uint8_t* s, uint8_t* action_array) {
...
/*----------------------------------------------------------------------------*/
  int run_action(packet_t* p, uint8_t* s, action_t* a) {
