...
create_data(uint8_t count) {
  ...
  #if DEBUG_MELO
    packet_t * create_data_MELO(int addressdest, uint8_t count) {
...
/*
  #if DEBUG_MELO
    packet_t * create_data_MELO1(int addressdest, uint8_t payload[], int len) {
      ...
      packet_t *pp = create_packet_payload(
      p->header.net,
      &p->header.dst,
      &p->header.src,
      p->header.typ,
...
*/
    ...
    packet_t * create_data_MELO1(int addressdest, int nodo, int n1, int n2, int n3, int dest,int nxhop) {
      packet_t *p = NULL;
        ...
        if (p != NULL) {
          ...
          p->header.net = 1;
          p->header.dst = get_address_from_int(2); // Replace 1 with your dst
          p->header.src = get_address_from_int(1);
          p->header.typ = DATA;
          p->header.nxh = get_address_from_int(2);
      ...
      p->header.nxh.u8[0] = nxhop;
      p->header.nxh.u8[1] = 0;
      p->header.dst.u8[0] = dest;
      p->header.dst.u8[1] = 0;
      ...
      packet_t * create_data_MELO2(uint8_t payload[], int dim, int dest,int nxhop) {
...
packet_t * create_reg_proxy(void) {
  ...
  #if DEBUG_MELO
    printf("packet-creator.c --- create_reg_proxy\n");
  #endif
  ...
  #if DEBUG_MELO
    printf("packet-creator.c --- create_reg_proxy - create_packet_payload ");
    printf(conf.my_net, &conf.sink_address, &conf.my_address, REG_PROXY, &conf.nxh_vs_sink, payload, 28);
    printf("\n");
  #endif
