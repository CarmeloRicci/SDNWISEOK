...
int serial_input_byte_melo(unsigned char c) {
    ...
    if (uart_buffer_index == 80) {
      countf++;
      printf("F: %d\n",countf);
      if (countf == 1) printf("Node: Ricevuto la prima frame\n");
      if (countf == 101) printf("Node: Ricevuto la frame 101\n");
      if (countf == 1001) printf("Node: Ricevuto la frame 1001\n");
      if (countf == 10101) printf("Node: Ricevuto la frame 10101\n");
      if (countf == 100101) printf("Node: Ricevuto la frame 100101\n");
      uart_buffer_index = 0;

      //packet_t *p = create_data_MELO2(uart_buffer,80);
      packet_t *p = create_data_MELO1(2,50, 48, 48, (48+countf),2,2);
      //packet_t *p = create_data_MELO2(uart_buffer,80,2,2);
      //print_packet_MELO(p);
      if (p != NULL) {
        p->info.rssi = 0;
...
void rf_unicast_send(packet_t *p) {
  //printf("--- rf_unicast_send - Mando un Pacchetto, sveglio rf_u_send_proc\n");
  process_post(&rf_u_send_proc, RF_U_SEND_EVENT, (process_data_t)p);
}
/*------------------------------------------------------------------------------*/
...
PROCESS_THREAD(main_proc, ev, data) {
    ...
    address_list_init();
    leds_init();
    printf("Iniziamo NODE\n");
    //printf("\nNODE Local Address: %d.%d\n",linkaddr_node_addr.u8[0] , linkaddr_node_addr.u8[1]);
...
PROCESS_THREAD(rf_u_send_proc, ev, data){
  ...
  while (1)  {
    ...
    //printf("--- rf_u_send_proc - Invio un pacchetto\n");
    ...
    if (p != NULL) {
      ...
      uint8_t *b = (uint8_t *)p;
      packetbuf_copyfrom(b, p->header.len);
      unicast_send(&uc, &addr);
      //printf("--- rf_u_send_proc - Inviato !!!!\n");

      counts++;
      if (counts == 1) printf("Node: Inviato la prima frame\n");
      if (counts == 101) printf("Node: Inviato la frame 101\n");
      if (counts == 1001) printf("Node: Inviato la frame 1001\n");
      if (counts == 10101) printf("Node: Inviato la frame 10101\n");
      if (counts == 100101) printf("Node: Inviato la frame 100101\n");
